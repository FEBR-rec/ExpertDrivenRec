...
class IEvVideo(document.AbstractDocument):
    ...
    def create_observation(self):
        ...
        return np.append(self.features,[self.quality, self.video_length])

    @classmethod
    def observation_space(cls):
        return spaces.Box(
            shape=(cls.NUM_FEATURES + 2,), dtype=np.float32, low=-1.0, high=1.0)
...
class IEvVideoSampler(document.AbstractDocumentSampler):
    ...
    def sample_document(self):
        ...
        doc_features['quality'] = np.random.uniform(-1, 1, 1)
        doc_features['cluster_id'] = 0

        #topic_id = np.random.choice([0, 1, 2], p=[0.1, 0.8, 0.1])

        topic_id = 0
        doc_features['features'] = np.zeros(self.get_doc_ctor().NUM_FEATURES)
        doc_features['features'][topic_id] = 1
...
class IEvUserState(user.AbstractUserState):
    ...
    def score_document(self, doc_obs):
        ...
        a = np.dot(self.user_interests, doc_obs[:self.NUM_FEATURES])
        return 1 / (1 + np.exp(-a))
...
class IEvUserModel(user.AbstractUserModel):
    ...
    def update_state(self, slate_documents, responses):
        ...
        for doc, response in zip(slate_documents, responses):
            if response.clicked:
                ...
                user_state.time_budget += (user_state.user_update_alpha * response.watch_time *
                        received_utility)
                return
    ...
    def simulate_response(self, documents):
        ...
        if selected_index is None:
            return responses, self.choice_model._scores
...
def clicked_watch_reward(responses):
...
def create_environment(env_config):
    ...
    document_sampler = IEvVideoSampler(
